...
Image::Image(const int rows, const int cols, const IoStrategy& s): rows_{rows}, cols_{cols}, strategy_{s}{
std::cout << "constructor 3 inputs called" << std::endl;
std::cout << "rows: " << rows  << " and cols: "<< cols << std::endl;
}

Image::Image(const IoStrategy& s): strategy_{s} {
std::cout << "constructor 1 input called" << std::endl;
}
...
image_data = strategy_.Read(file_name); 
...
strategy_.Write(file_name, image_data); 
